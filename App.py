...
class Application:
    def __init__(self,master=None):
            ...
            self.titulo["font"] = ("Calibre","9","bold")
            self.titulo.pack()
    ...
    def alterarUsuario(self):
         ...
         self.cursor.execute("UPDATE usuario SET nome=%s,telefone=%s,email=%s,usuario=%s,senha=%s WHERE id_usuario=%s",
    ...
    def excluirUsuario(self):
         ...
         self.conn.commit()
         self.lblmsg["text"]="Usuario deletado com sucesso"
         self.limparCampos() 

    def buscarUsuario(self):
         id_usuario = self.txtidusuario.get()
         self.cursor.execute("SELECT * FROM usuario WHERE id_usuario=%s",
                             (id_usuario,))
